document.querySelectorAll('a[href^="#"]').forEach(link => {
  link.addEventListener('click', e => {
    ...
    if (musicBtn && bgMusic) {
      console.log("Music Player Initialized. Audio Source:", bgMusic.src);
      ...
      function toggleMusic() {
        if (bgMusic.paused) {
          ...
          bgMusic.play().then(() => {
            ...
            console.error("Playback failed via button:", err);
          });
        } else {
      ...
      bgMusic.addEventListener('error', (e) => {
        console.error("Audio error event:", bgMusic.error);
        if (bgMusic.error.code === 4) {
          console.error("Error 4: Source not supported or file not found.");
        }
      });

      bgMusic.addEventListener('loadstart', () => console.log("Audio started loading..."));
      bgMusic.addEventListener('canplaythrough', () => console.log("Audio can play through."));

      const startOnFirstInteraction = () => {
        if (bgMusic.paused) {
          bgMusic.play().then(() => {
            console.log("Playback started via interaction.");
            updateMusicUI();
            removeInteractionListeners();
          }).catch(err => {
            console.warn("Autoplay still blocked or failed:", err.message);
          });
        } else {
          removeInteractionListeners();
        }
    ...
    /* ── PREVENT DROPDOWN LINKS FROM TRIGGERING MUSIC ── */
    document.querySelectorAll('.dropdown a').forEach(link => {
      link.addEventListener('click', (e) => {
        // Mencegah trigger musik otomatis pada interaksi pertama jika mengklik sub-menu
        e.stopPropagation();
      });
    });

    /* ── PREVENT CHATBOT CLICK FROM TRIGGERING MUSIC ── */
    const chatLauncher = document.getElementById('zapierChatLauncher');
    if (chatLauncher) {
      ['click', 'touchstart'].forEach(evt => {
        chatLauncher.addEventListener(evt, (e) => {
          // This prevents the global document listeners from starting the music
          e.stopPropagation();
        });
      });
    }
